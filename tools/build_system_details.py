...
def build():
    ...
    meta = [
        ...
        ("Current status", "Working React/Vite + Node local MVP with encrypted local persistence"),
    ...
    add_table(doc, ["Verification area", "Current evidence / next check"], [
        ("Automated tests", "28 Node tests currently pass across domain, auth, qualification and upload-policy suites. Extend with document magic-byte fixtures, cross-organisation denial and notification status failure/retry cases."),
        ("Build", "npm run build passes after the active frontend was moved off mockStore.js. Re-run after any demo UI change."),
    ...
    add_body(doc, "The demo shortcuts intentionally remain in the local login dialog for tomorrow's presentation. They use the demo endpoint and must be disabled by setting GCON_AUTH_MODE to a non-demo production mode and configuring the external identity adapter.")
    ...
    for item in [
        ...
        "After API/database acceptance, remove demo-only data paths and prototype-only components from the production build. Keep the demo shortcuts only in the isolated presentation/local configuration until the demo is complete.",
    ]:
        add_number(doc, item)

...
def build():
    ...
    add_table(doc, ["Area", "Current capability", "Current boundary"], [
        ("Learner portal", "Landing page, configurable intake banner, qualification checker, three pathway forms, profile, preferences, address confirmation, evidence upload, review, receipt, correction request and final outcome views.", "Local demo profile values and local draft persistence remain. Production identity and policy configuration are still required."),
        ("Staff workspace", "Dashboard, submitted applications, review queue, per-document verify/reject/request-correction actions, controlled application decisions, shortlist, psychometric invitation and outcome, communications, placements, withdrawals, termination-letter recording, reports and audit.", "The API is a Node local service rather than the planned NestJS/PostgreSQL production stack. Some seeded demo records have already been used during QA."),
        ("Employer portal", "Organisation-scoped dashboard, released candidates, placement offer responses and placement state visibility.", "Employer access is limited to released/assigned GCON records. Future hospitals and clinics can be added through the organisation/membership model."),
        ("Access control", "Real email/password local sessions, expiry handling, invitations, membership-derived roles, logout, plus retained local demo shortcuts for the presentation.", "Local passwords are development seeds. Production must use Microsoft Entra External ID or an equivalent managed identity provider."),
    ...
    for item in [
        ...
        "Run the qualification checker before full registration. Supported pathways are Senior Certificate / Standard 10, National Senior Certificate / Grade 12, and NC(V) Level 4.",
        "If the learner qualifies, continue to the pathway-specific profile and application form. If not, only the minimum contact request is retained; qualification evidence is not kept by the non-qualifier contact function.",
        "Capture personal details, South African ID, structured Gauteng address, school, result year, preferences, previous nursing training, work experience, references and the two approved evidence documents.",
        "Upload real PDF, JPG or PNG files through an API upload intent, direct content upload and completion/checksum step. The learner sees a saved draft and can return later.",
    ...
    for item in [
        ...
        "Record the controlled application decision: approve for shortlist, request correction, decline with a controlled reason, or undo where allowed by the current workflow.",
    ...
    add_body(doc, "The current frontend demo is published privately at https://gcon-nursing-intake.alistairljohanson.chatgpt.site. The Sites deployment uses the compiled Vite assets plus dist/server/index.js, a Cloudflare-compatible worker generated by tools/build-sites-worker.mjs. The hosted worker serves the learner, admissions and employer presentation surfaces and provides a deliberately limited demo API with seeded state so the retained role pickers continue to work outside the local machine.")
    add_callout(doc, "Hosted demo boundary", "The Sites demo is not connected to the local encrypted API, PostgreSQL, Azure Blob Storage or local uploaded documents. Hosted state-changing actions are not a production persistence boundary. Use the local app for API, real-file upload, audit, database and document-storage acceptance. The Sites project is private to the owning ChatGPT account until its access policy is deliberately changed.", fill="FFF8E8", label_color=GOLD)
    add_table(doc, ["Layer", "Current implementation", "Production target"], [
        ...
        ("Hosted demo", "Private ChatGPT Sites deployment at gcon-nursing-intake.alistairljohanson.chatgpt.site. npm run build generates the static/demo worker and .openai/hosting.json stores only the Sites project ID.", "Deploy the approved production frontend against the production API, managed identity, database, private document storage and external notification services."),
    ], [1800, 3600, 3960])
    ...
    add_table(doc, ["Record", "Important fields / purpose"], [
        ...
        ("Intake cycle", "Cycle ID, name/season, open date, close date, advert status, reference date, policy status and document labels."),
        ("Application", "Reference, owner, pathway, profile, preferences, original pathway values, status, submittedAt, correction request, outcome and placement information."),
    ...
    add_table(doc, ["Area", "Routes currently implemented"], [
        ("Health / contract", "GET /v1/health; GET /v1/openapi.json; GET /v1/storage/status"),
        ("Authentication", "POST /v1/auth/login; POST /v1/auth/logout; GET /v1/auth/session; POST /v1/auth/invitations; GET /v1/auth/invitations; POST /v1/auth/invitations/accept; POST /v1/auth/demo-login"),
        ("Intake", "GET/PATCH /v1/intake/current"),
        ("Learner application", "GET/PATCH /v1/applications/me; POST /v1/applications/submit; POST /v1/applications/non-qualifier"),
        ...
        ("Staff review", "GET /v1/reviews/queue; POST /v1/reviews/{ref}/decision; POST /v1/shortlists"),
    ...
    for item in [
        ...
        "Eligibility wording: pathway names, approved subject labels, thresholds, APS/M score rules and NC(V) percentage rules, with an approval state and effective version.",
    ...
    add_table(doc, ["Verification area", "Current evidence / next check"], [
        ("Automated tests", "30 Node tests currently pass across domain, auth, qualification and upload-policy suites. Extend with document magic-byte fixtures, cross-organisation denial and notification status failure/retry cases."),
    ...
    for item in [
        "Confirm the official closing date, post-submission edit policy, exact document labels, employer permissions, retention schedule, support owner, sender identity, Azure subscription and region, and approved qualification wording.",
    ...
    add_table(doc, ["File / command", "Purpose"], [
        ...
        ("npm test", "Run Node domain/auth/qualification/upload tests."),

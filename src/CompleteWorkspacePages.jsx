...
export function CompleteReviewQueue({ selectedRef: initialRef, setSelectedRef, data: liveData, onDataChange }) {
  ...
  const applications = (data.applications || []).filter((application) => reviewStatuses.includes(application.status) && (application.status === 'Under review' || application.correctionResubmittedAt)).sort((left, right) => (left.academicRank ?? 999) - (right.academicRank ?? 999) || String(left.name).localeCompare(String(right.name)));
...
const defaultCommunicationMessages = Object.freeze({
  'Psychometric test invitation': 'Dear {{name}} {{surname}}, your psychometric assessment invitation is available in your applicant communication history for ID {{id}}.',
  'Shortlist confirmation': 'Dear {{name}} {{surname}}, your application for ID {{id}} has been approved for the next evaluation stage.',
  'Decline outcome': 'Dear {{name}} {{surname}}, your application outcome for ID {{id}} is available. Please review the recorded reason in your applicant portal.',
  'Termination / withdrawal': 'Dear {{name}} {{surname}}, this letter records the withdrawal or termination decision against ID {{id}}.',
});
...
function previewCommunicationMessage(message) {
  return String(message || '')
    .replaceAll('{{name}}', 'Lerato')
    .replaceAll('{{surname}}', 'Mokoena')
    .replaceAll('{{id}}', '990101•••••081')
    .replaceAll('{{ID}}', '990101•••••081')
    .replaceAll('{{idNumber}}', '990101•••••081');
}
...
export function CompleteReportsPage({ data: liveData, onRefresh }) {
  ...
  async function refreshReport() {
    ...
    if (remote) setRefreshedAt(new Date());
    setRefreshing(false);
  }
  ...
  return <section className="staff-page"><div className="page-heading"><div><p className="eyebrow">REPORTS · {data.cycle?.name || 'GCON 2027'}</p><h1>Intake reporting</h1><p>Live counts from the current application and workflow state.</p></div><div className="page-heading-actions"><span className="saved-label">{refreshedAt ? `Updated ${refreshedAt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : 'Live workspace data'}</span><button className="outline-button" type="button" onClick={refreshReport} disabled={refreshing}>{refreshing ? 'Refreshing…' : 'Refresh report'} <span>↻</span></button><button className="outline-button" type="button" onClick={exportReport}>Export cycle report <span>↓</span></button></div></div><div className="report-grid"><div className="panel report-card"><span>Complete applications</span><strong>{complete}</strong><div className="report-bar"><i style={{ width: `${Math.min(100, complete * 20)}%` }} /></div><small>Drafts are excluded from the cycle total</small></div><div className="panel report-card"><span>Shortlist rate</span><strong>{passRate}%</strong><div className="report-bar gold"><i style={{ width: `${passRate}%` }} /></div><small>Based on live submitted records</small></div><div className="panel report-card"><span>Shortlisted or placed</span><strong>{shortlisted}</strong><div className="report-bar green"><i style={{ width: `${complete ? (shortlisted / complete) * 100 : 0}%` }} /></div><small>Eligibility and placement remain separate actions</small></div></div><div className="panel preference-report"><div className="panel-heading"><div><h2>College preference demand</h2><p>Selections recorded in current application records.</p></div><span className="saved-label">{data.cycle?.advertStatus || 'Published'}</span></div>{counts.map(([campus, count]) => <div className="demand-row" key={campus}><span>{campus}</span><strong>{count}</strong><div><i style={{ width: `${complete ? (count / complete) * 100 : 0}%` }} /></div></div>)}</div></section>;
}
...
export function MassApplicationsPage({ setSelectedRef, setPage }) {
  ...
  const applications = (data.applications || []).filter((application) => application.status !== 'Draft');
  const visible = applications.filter((application) => `${application.ref} ${application.name} ${application.id} ${application.pathway}`.toLowerCase().includes(query.toLowerCase()) && (pathway === 'All pathways' || application.pathway === pathway));
  const rankedVisible = [...visible].sort((left, right) => (left.academicRank ?? 999) - (right.academicRank ?? 999) || String(left.name).localeCompare(String(right.name))).slice(0, 8);
  ...
  const exportRows = [['Applicant', 'Reference', 'Pathway', 'Reported academic score', 'Score type', 'Pathway rank', 'Status'], ...visible.map((application) => [application.name, application.ref, application.pathway, application.academicScore || application.score, application.academicScoreLabel || 'Reported score', application.academicRank ? `#${application.academicRank}` : '', application.status])];
  ...
  return <section className="staff-page"><div className="page-heading"><div><p className="eyebrow">APPLICATIONS</p><h1>All submitted applications</h1><p>Search, filter, and open a record for human review. The academic ranking uses each pathway's recorded score without converting between pathways.</p></div><button className="outline-button" onClick={() => downloadCsv('gcon-2027-applications.csv', exportRows)}>Export report <span>Download</span></button></div>{message && <div className="decision-banner approved mass-decline-message" role="status"><span>OK</span><div><strong>{message}</strong><small>Each outcome is linked to the in-app notification and audit trail.</small></div></div>}{confirming && <div className="mass-decline-confirm" role="alertdialog" aria-labelledby="mass-decline-title"><div><p className="eyebrow">BULK ACTION</p><h2 id="mass-decline-title">Decline {selectedRefs.length} applications?</h2><p>This will set each selected application to Declined and create one in-app outcome notification per learner. The action is audit-linked and cannot be applied to shortlisted or placed records.</p></div><label className="field"><span>Reason recorded for every selected learner</span><select value={reason} onChange={(event) => setReason(event.target.value)}><option>Required subject or score not met</option><option>Incorrect or unreadable document</option><option>Document type not supplied</option><option>Information could not be verified</option></select></label><div className="form-actions compact-actions"><button className="quiet-button" type="button" onClick={() => setConfirming(false)} disabled={busy}>Cancel</button><button className="decline-button" type="button" onClick={massDecline} disabled={busy}>{busy ? 'Declining...' : `Confirm mass decline (${selectedRefs.length})`}</button></div></div>}<div className="panel academic-ranking-panel"><div className="panel-heading"><div><h2>Highest academic ratings</h2><p>Top applicants in the current view, ranked within their pathway using the reported academic value.</p></div><span className="saved-label">Human review still required</span></div><div className="academic-ranking-grid">{rankedVisible.map((application) => <button className="academic-ranking-row" type="button" key={application.ref} onClick={() => { setSelectedRef?.(application.ref); setPage('reviewQueue'); }}><span className="academic-rank-badge">#{application.academicRank || '—'}</span><span className="academic-ranking-person"><strong>{application.name}</strong><small>{application.ref} · {application.pathway}</small></span><span className="academic-ranking-score"><strong>{application.academicScore || application.score}</strong><small>{application.academicScoreLabel || 'Reported score'}</small></span><LocalStatus status={application.status} /></button>)}{rankedVisible.length === 0 && <div className="empty-table">No applicants match the current search.</div>}</div></div><div className="panel work-panel full-table"><div className="table-toolbar"><div className="search wide"><span className="icon">Search</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search surname, ID number or reference" /></div><span className="results-count">{visible.length} visible results</span><select className="filter-button" value={pathway} onChange={(event) => setPathway(event.target.value)}><option>All pathways</option><option>NSC / Grade 12</option><option>Senior Certificate</option><option>NC(V) Level 4</option></select><button className="decline-button" type="button" disabled={!selectedRefs.length} onClick={() => setConfirming(true)}>Mass decline{selectedRefs.length ? ` (${selectedRefs.length})` : ''}</button></div><div className="mass-action-hint"><span>{selectedRefs.length ? `${selectedRefs.length} selected for mass decline` : 'Select applications under review to send linked in-app outcomes.'}</span><small>{declinable.length} eligible on this page</small></div><div className="table-wrap"><table><thead><tr><th className="bulk-select-cell"><input type="checkbox" aria-label="Select all applications eligible for mass decline" checked={allVisibleSelected} onChange={toggleAll} /></th><th>Applicant</th><th>Reference</th><th>Pathway</th><th>Academic rank</th><th>Reported score</th><th>Status</th><th>Updated</th><th /></tr></thead><tbody>{visible.map((application) => { const canMassDecline = ['Under review', 'Correction requested'].includes(application.status); return <tr key={application.ref} onClick={() => { setSelectedRef?.(application.ref); setPage('reviewQueue'); }}><td className="bulk-select-cell" onClick={(event) => event.stopPropagation()}><input type="checkbox" aria-label={`Select ${application.name} for mass decline`} checked={selectedRefs.includes(application.ref)} disabled={!canMassDecline} onChange={() => toggleRef(application.ref)} /></td><td><strong>{application.name}</strong><small>{application.id}</small></td><td>{application.ref}</td><td>{application.pathway}</td><td><strong>{application.academicRank ? `#${application.academicRank}` : '—'}</strong></td><td><strong>{application.academicScore || application.score}</strong><small>{application.academicScoreLabel || 'Reported score'}</small></td><td><LocalStatus status={application.status} /></td><td>{application.updated}</td><td><button className="row-arrow" aria-label={`Open ${application.ref}`}>Open</button></td></tr>; })}{visible.length === 0 && <tr><td colSpan="9" className="empty-table">No matching applications.</td></tr>}</tbody></table></div></div></section>;
}

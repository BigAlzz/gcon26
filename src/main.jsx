...
function AccessManagement({ result, profile, session, onRegister, setStep }) {
  ...
  async function createAccount() {
    ...
    setBusy(true);
    const response = await onRegister({ username, password, name: `${profile.firstName || 'Applicant'} ${profile.surname || ''}`.trim(), email: profile.email || '' });
    setBusy(false);
    if (!response?.user) { setError(response?.error?.message || 'The account could not be created.'); return; }
    setStep('profile');
  }
  return <div className="form-page"><StepHeader current="access" /><div className="form-grid"><section className="form-card access-card"><div className="card-heading"><div><h2>Access management</h2><p>Create your applicant account before completing the registration form.</p></div><span className="required-note">Required</span></div>{session ? <div className="access-active"><div className="result-mark">✓</div><div><strong>Your applicant account is active</strong><p>You are signed in securely. Continue to complete your profile.</p></div><button className="primary-button" type="button" onClick={() => setStep('profile')}>Continue to profile <span>→</span></button></div> : <><p className="access-note">Your username is your South African ID number. It is shown here for account identification and cannot be changed on this page.</p><label className="field"><span>Username</span><input value={username} readOnly /></label><div className="field-grid two"><label className="field"><span>Password</span><input type="password" autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} /></label><label className="field"><span>Confirm password</span><input type="password" autoComplete="new-password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} /></label></div>{error && <p className="login-error" role="alert">{error}</p>}<button className="primary-button form-submit" type="button" disabled={busy || !username} onClick={createAccount}>{busy ? 'Creating account…' : 'Create applicant account'} <span>→</span></button></>}</section><aside className="side-explain"><div className="side-icon">✓</div><h3>One account for your application</h3><p>Your account lets you save the registration draft, upload documents, and return to your application securely.</p><div className="criteria-mini"><strong>Privacy boundary</strong><span>Passwords are stored as secure hashes. The full password is never placed in the application record.</span></div></aside></div></div>;
}
...
function NonQualifierCardForm({ setResult, result }) {
  ...
  async function saveContact() {
    setError('');
    const response = await recordApiNonQualifier({ ...contact, pathway: result?.pathway, idNumber });
    if (!response) { setError('We could not save your contact details. Please try again.'); return; }
    setSaved(true);
  }
  return <div className="nonqualifier-card"><div className="result-mark">!</div><div><strong>We could not confirm the minimum criteria</strong><p>Your selected certificate and ID are shown for your reference. Only your name, email and telephone number are retained for future intake requirements.</p></div><div className="nonqualifier-summary"><div><span>Selected certificate</span><strong>{result?.pathway || 'Not supplied'}</strong></div><div><span>South African ID number</span><strong>{maskedId}</strong></div></div><div className="minimal-contact"><label className="field"><span>Name</span><input value={contact.name} onChange={(event) => update('name', event.target.value)} /></label><label className="field"><span>Email</span><input type="email" value={contact.email} onChange={(event) => update('email', event.target.value)} /></label><label className="field"><span>Telephone</span><input type="tel" value={contact.telephone} onChange={(event) => update('telephone', event.target.value)} /></label></div>{error && <p className="login-error">{error}</p>}{saved ? <div className="decision-banner approved"><span>✓</span><div><strong>Contact details saved</strong><small>We retained only the minimum contact details for future intake requirements.</small></div></div> : <button className="primary-button" type="button" onClick={saveContact}>Keep my contact details</button>}<button className="quiet-button" type="button" onClick={() => setResult(null)}>Review my results again</button></div>;
}
...
function ProfileFormComplete({ cycle, preferences, setPreferences, profile, setProfile, pathway, qualificationResult, addressConfirmed, setAddressConfirmed, documents, setDocuments, references, setReferences, previousTraining, setPreviousTraining, experience, setExperience, additionalSubjects, setAdditionalSubjects, trainingHistory, setTrainingHistory, canReview, setStep, applicationRef }) {
  ...
  return <div className="form-page"><StepHeader current="profile" /><div className="profile-layout"><section className="form-card"><div className="card-heading"><div><h2>Complete your profile</h2><p>Your qualification values stay in their original reported form and the fields below are saved as your draft.</p></div><span className="saved-label">Saved automatically</span></div>
    ...
    <div className="form-section"><div className="section-line"><h3>School details and college preferences</h3><span className="required-note">Rank 1-3</span></div><div className="field-grid two">{input('School', 'school')}{input('Year of results', 'resultYear', 'number')}</div><div className="subsection-line"><strong>Remaining subjects</strong><span>{pathway}-specific subjects not used in the minimum check</span></div>{additionalSubjects.map((subject, index) => <div className="subject-entry" key={`subject-${index}`}><input value={subject.name} onChange={(event) => updateSubject(index, 'name', event.target.value)} placeholder="Subject name" /><input value={subject.result} onChange={(event) => updateSubject(index, 'result', event.target.value)} placeholder="Result / level" /></div>)}<button className="quiet-button add-button" type="button" onClick={addSubject} disabled={additionalSubjects.length >= 12}>+ Add remaining subject</button>{preferences.map((preference, index) => <div className="preference-input" key={`preference-${index}`}><span>Preference {index + 1}</span><select value={preference} onChange={(event) => updatePreference(index, event.target.value)}><option value="">No preference selected</option>{collegeCampuses.filter((campus) => !preferences.includes(campus) || campus === preference).map((campus) => <option key={campus}>{campus}</option>)}</select><small>Selection count is recorded for capacity planning.</small></div>)}</div>
...
function QualificationCheckerV2({ pathway, setPathway, result, setResult, checkQualification, setStep }) {
  ...
  const percentageSelect = (label, key, minimum) => select(label, key, ['Below requirement', ...Array.from({ length: 100 - minimum + 1 }, (_, index) => String(minimum + index))]);
  ...
  const ncvFundamentalControls = [percentageSelect('English First Additional Language (%)', 'englishFal', 50), percentageSelect('Maths / Maths Literacy (%)', 'mathematics', 50), percentageSelect('Life Orientation (%)', 'lifeOrientation', 50)];
  const ncvVocationalControls = [percentageSelect('SA Health Care System (%)', 'saHealthCare', 60), percentageSelect('Public Health (%)', 'publicHealth', 60), percentageSelect('The Human Body and Mind (%)', 'humanBody', 60), percentageSelect('Community Oriented Primary Care (%)', 'communityPrimaryCare', 60)];

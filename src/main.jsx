...
const subjectChoicesByStream = Object.freeze({
  'NSC / Grade 12': {
    tab: 'NSC',
    options: ['Advanced Programme Mathematics', 'Mechanical Technology', 'Accounting', 'Agricultural Management Practices', 'Agricultural Sciences', 'Agricultural Technology', 'Art', 'Business Studies', 'Civil Technology', 'Computer Applications Technology', 'Consumer Studies', 'Dance Studies', 'Design', 'Dramatic Arts', 'Economics', 'Electrical Technology', 'Engineering Graphics & Design', 'Geography', 'History', 'Hospitality Studies', 'Information Technology', 'Life Orientation', 'Life Sciences', 'Mathematical Literacy', 'Mathematics', 'Music', 'Physical Science', 'Religion Studies', 'Tourism', 'Visual Arts', 'Afrikaans', 'English', 'isiNdebele', 'Sepedi', 'Sesotho', 'siSwati', 'itsonga', 'Setswana', 'Tshivenḓa', 'isiXhosa', 'isiZulu', 'Other Subjects'],
  },
  'Senior Certificate': {
    tab: 'SENIOR',
    options: ['English', 'Afrikaans', 'Biology', 'Mathematics', 'Physical Science', 'Geography', 'History', 'Home Economics', 'Agricultural Science', 'Biblical Studies', 'Technical Drawing', 'Business Economics', 'Typing', 'Accounting', 'Other'],
  },
  'NC(V) Level 4': {
    tab: 'VOCATIONAL',
    options: ['English', 'Maths/Maths Literacy', 'Life Orientation', 'The SA Health Care System', 'The Human Body and Mind', 'Public Health', 'Community Oriented Primary Care'],
  },
});
...
function NonQualifierCardForm({ setResult, result, recordNonQualifier }) {
  const [contact, setContact] = useState(() => loadApplicantDraft().nonQualifierContact || { name: 'Lerato Mokoena', email: 'lerato.mokoena@email.com', telephone: '082 555 0194' });
  ...
  useEffect(() => {
    const draft = loadApplicantDraft();
    if (typeof window !== 'undefined') window.localStorage.setItem('gcon-learner-draft', JSON.stringify({ ...draft, nonQualifierContact: contact, nonQualifierResult: result }));
  }, [contact, result]);
  async function saveContact() {
    setError('');
    if (!contact.name.trim() || !contact.email.trim() || !contact.telephone.trim()) {
      setError('Name, email, and telephone are required to save your details.');
      return;
    }
    const saveRecord = recordNonQualifier || recordApiNonQualifier;
  ...
  return <div className="nonqualifier-card"><div className="result-mark">!</div><div><strong>We could not confirm the minimum criteria</strong><p className="nonqualifier-reason">Below required APS minimum score of 27, M score minimum of 17.</p><p>Save your details before leaving this page. Your qualification result is saved locally as you type, and the intake team can retain your contact request after you select Save my details.</p></div><div className="nonqualifier-summary"><div><span>Selected certificate</span><strong>{result?.pathway || 'Not supplied'}</strong></div><div><span>South African ID number</span><strong>{maskedId}</strong></div></div><div className="minimal-contact"><label className="field"><span>Name <b>*</b></span><input required value={contact.name} onChange={(event) => update('name', event.target.value)} /></label><label className="field"><span>Email <b>*</b></span><input required type="email" value={contact.email} onChange={(event) => update('email', event.target.value)} /></label><label className="field"><span>Telephone <b>*</b></span><input required type="tel" value={contact.telephone} onChange={(event) => update('telephone', event.target.value)} /></label></div>{error && <p className="login-error" role="alert">{error}</p>}{saved ? <div className="decision-banner approved"><span>✓</span><div><strong>Details saved as a non-qualifying record</strong><small>Your entered qualification values and contact details were recorded for this intake.</small></div></div> : <button className="primary-button" type="button" onClick={saveContact}>Save my details</button>}<button className="quiet-button" type="button" onClick={() => setResult(null)}>Review my results again</button></div>;
}

function NonQualifierCardComplete({ setResult, result, recordNonQualifier }) { return <NonQualifierCardForm setResult={setResult} result={result} recordNonQualifier={recordNonQualifier} />; }
...
function ProfileFormComplete({ cycle, preferences, setPreferences, profile, setProfile, pathway, qualificationResult, addressConfirmed, setAddressConfirmed, documents, setDocuments, references, setReferences, previousTraining, setPreviousTraining, experience, setExperience, additionalSubjects, setAdditionalSubjects, trainingHistory, setTrainingHistory, canReview, setStep, applicationRef }) {
  ...
  const removeSubject = (index) => setAdditionalSubjects(additionalSubjects.filter((_, itemIndex) => itemIndex !== index));
  const subjectStream = subjectChoicesByStream[pathway] || subjectChoicesByStream['NSC / Grade 12'];
  ...
  const input = (label, key, type = 'text') => {
    ...
    return <label className="field" key={key} htmlFor={inputId}><span>{label} <b>*</b></span><input id={inputId} name={key} aria-label={label} aria-required="true" required type={inputType} {...(contactField ? { defaultValue: profile[key] ?? '' } : { value: profile[key] ?? '' })} onInput={handleInput} /></label>;
  };
  ...
  return <div className="form-page"><StepHeader current="profile" /><div className="profile-layout"><section className="form-card"><div className="card-heading"><div><h2>Complete your profile</h2><p>Your qualification values stay in their original reported form and the fields below are saved as your draft.</p></div><span className="saved-label">Saved automatically</span></div>
    <div className="form-section"><h3>Personal details</h3><div className="field-grid three">{input('First name', 'firstName')}{input('Surname', 'surname')}{input('Email address', 'email', 'email')}</div><div className="field-grid two">{input('Mobile / cell number', 'mobile', 'tel')}{input('South African ID number', 'idNumber')}</div></div>
    <div className="form-section"><div className="section-line"><h3>Gauteng address</h3><span className={`verified-chip ${addressConfirmed ? '' : 'unverified-chip'}`}>{addressConfirmed ? 'Map location verified' : 'Pin needs confirmation'}</span></div><div className="field-grid address-fields">{input('Street address', 'streetAddress')}{input('Suburb / town', 'suburb')}<label className="field" htmlFor="profile-province"><span>Province <b>*</b></span><select id="profile-province" name="province" aria-label="Province" aria-required="true" required value={profile.province || ''} onChange={(event) => { updateProfile('province', event.target.value); setAddressConfirmed(false); }}><option>Gauteng</option><option>Other province</option></select></label>{input('Postcode', 'postcode')}</div><AddressMapPicker profile={profile} setProfile={setProfile} addressConfirmed={addressConfirmed} setAddressConfirmed={setAddressConfirmed} /></div>
    <div className="form-section"><div className="section-line"><h3>School details and college preferences</h3><span className="required-note">Rank 1-4</span></div><div className="field-grid two">{input('School', 'school')}<label className="field" htmlFor="profile-resultYear"><span>Year of results <b>*</b></span><select id="profile-resultYear" name="resultYear" aria-label="Year of results" aria-required="true" required value={profile.resultYear || ''} onChange={(event) => updateProfile('resultYear', event.target.value)}>{resultYearOptions.map((year) => <option key={year}>{year}</option>)}</select></label></div><div className="subsection-line"><strong>Remaining subjects</strong><span>{pathway}-specific subjects not used in the minimum check · choices from {subjectStream.tab} tab</span></div>{additionalSubjects.map((subject, index) => <div className="subject-entry" key={`subject-${index}`}><select value={subject.name} onChange={(event) => updateSubject(index, 'name', event.target.value)} aria-label={`Subject ${index + 1}`}><option value="">Select subject</option>{subjectStream.options.map((option) => <option key={option}>{option}</option>)}{subject.name && !subjectStream.options.includes(subject.name) && <option value={subject.name}>{subject.name}</option>}</select><input value={subject.result} onChange={(event) => updateSubject(index, 'result', event.target.value)} placeholder="Result / level" aria-label={`Result / level for subject ${index + 1}`} /><button className="subject-remove" type="button" onClick={() => removeSubject(index)} aria-label={`Remove subject ${index + 1}`}>Remove</button></div>)}<button className="quiet-button add-button" type="button" onClick={addSubject} disabled={additionalSubjects.length >= 12}>Submit / Add Remaining Subject</button>{preferences.map((preference, index) => <div className="preference-input" key={`preference-${index}`}><span>Preference {index + 1}</span><select id={`profile-preference-${index + 1}`} name={`preference-${index + 1}`} aria-label={`Preference ${index + 1}`} value={preference} onChange={(event) => updatePreference(index, event.target.value)}><option value="">No preference selected</option>{collegeCampuses.filter((campus) => !preferences.includes(campus) || campus === preference).map((campus) => <option key={campus}>{campus}</option>)}</select><small>Selection count is recorded for capacity planning.</small></div>)}</div>
    ...
    <div className="form-section"><div className="section-line"><h3>References</h3><span className="required-note">1-3 mandatory</span></div><p className="section-copy">Add at least one reference with a relationship and telephone number.</p>{references.map((reference, index) => <div className="reference-row" key={`reference-${index}`}><input id={`reference-name-${index + 1}`} name={`reference-name-${index + 1}`} aria-label={`Reference ${index + 1} name`} value={reference.name} onChange={(event) => updateReference(index, 'name', event.target.value)} placeholder="Reference name" /><select id={`reference-relationship-${index + 1}`} name={`reference-relationship-${index + 1}`} aria-label={`Reference ${index + 1} relationship`} value={reference.relationship} onChange={(event) => updateReference(index, 'relationship', event.target.value)}>{relationshipOptions.map((relationship) => <option key={relationship}>{relationship}</option>)}</select><input id={`reference-telephone-${index + 1}`} name={`reference-telephone-${index + 1}`} aria-label={`Reference ${index + 1} telephone`} defaultValue={reference.telephone || ''} onInput={(event) => updateReference(index, 'telephone', event.target.value)} placeholder="Telephone" /></div>)}<button className="quiet-button add-button" type="button" onClick={addReference} disabled={references.length >= 3}>+ Add another reference</button></div>
     <div className="form-actions"><button className="quiet-button" type="button" onClick={() => setStep('checker')}>Back</button><div className="form-submit-group"><span>{canReview && referenceReady ? 'Ready for review and final submission' : 'Complete your profile, confirm Gauteng, upload both documents, and add one reference'}</span><button className="primary-button" type="button" disabled={!canReview || !referenceReady} onClick={() => setStep('review')}>Review / submit application <span>→</span></button></div></div>
...
function QualificationCheckerV2({ pathway, setPathway, result, setResult, checkQualification, recordNonQualifier, setStep }) {
  ...
  return <div className="form-page"><StepHeader current="checker" /><div className="form-grid"><section className="form-card checker-card"><div className="card-heading"><div><h2>Check your qualification</h2><p>Choose the certificate pathway and confirm only the criteria that apply to it.</p></div><span className="required-note">Required</span></div><div className="pathway-tabs">{pathways.map((item) => <button className={pathway === item ? 'selected' : ''} key={item} onClick={() => setPathway(item)}>{item}</button>)}</div><p className="subject-source-note"><strong>School subject choices:</strong> {subjectStream.tab} tab. Select additional subjects from this stream in your profile.</p><div className="field-grid two qualification-identity-grid"><label className="field"><span>South African ID number</span><input required aria-required="true" value={values.idNumber || ''} onChange={(event) => update('idNumber', event.target.value)} /></label><label className="field"><span>Year of results</span><select required aria-required="true" value={values.resultYear || ''} onChange={(event) => update('resultYear', event.target.value)}>{resultYearOptions.map((year) => <option key={year}>{year}</option>)}</select></label></div>{pathway === 'NC(V) Level 4' ? <div className="qualification-groups"><div className="qualification-group"><h3>Fundamental Subjects</h3><p>English First Additional Language, Maths/Maths Literacy, and Life Orientation.</p><div className="field-grid two">{ncvFundamentalControls}</div></div><div className="qualification-group"><h3>Vocational Subjects</h3><p>Use the named NC(V) nursing-related subjects below.</p><div className="field-grid two">{ncvVocationalControls}</div></div></div> : <div className="field-grid two">{controls}</div>}{pathway === 'NSC / Grade 12' && <p className="subject-note">Enter either Mathematics or Maths Literacy. Only one mathematics result is used in a score calculation; they are never added together.</p>}<div className="form-footnote"><span className="secure-dot" /> Original reported values are preserved. No percentage-to-APS or certificate equivalency is invented.</div>{result?.status === 'qualifies' && <QualificationResultCard result={result} setStep={setStep} />}{result?.status === 'does-not-qualify' && <NonQualifierCardComplete setResult={setResult} result={result} recordNonQualifier={recordNonQualifier} />}{!result && <><button className="primary-button form-submit" onClick={evaluate}>Check qualification <span>→</span></button><button className="checker-secondary" onClick={markNotQualified}>I do not meet all of the criteria</button></>}</section><aside className="side-explain"><div className="side-icon">✓</div><h3>Why we ask this first</h3><p>Your qualification result determines whether you continue to registration. If you do not qualify, we store the entered qualification values and non-qualifying result with any details you provide. Save your details before leaving this page.</p><div className="criteria-mini"><strong>{pathway} rule</strong><span>{pathway === 'Senior Certificate' ? 'English, Biology and Mathematics at HG D or SG C, plus M score 17.' : pathway === 'NC(V) Level 4' ? 'Fundamentals at 50%+ and 60%+ in each named vocational subject.' : 'English and Life Sciences Level 4+, Maths Level 4 or Maths Literacy Level 5+, reported APS 27+.'}</span></div><div className="criteria-mini"><strong>Gauteng residence</strong><span>Your structured address and map pin must be within Gauteng.</span></div></aside></div></div>;
}

function QualificationResultCard({ result, setStep }) { return <div className="result-card"><div className="result-mark">✓</div><div><strong>{result.pathway === 'NSC / Grade 12' ? 'Subject criteria recorded for review' : `You meet the ${result.pathway} minimum criteria`}</strong><p>{result.pathway === 'Senior Certificate' ? <>Calculated M score: <b>{result.score}</b> · Based on the three submitted HG/SG subject grades.</> : result.pathway === 'NSC / Grade 12' ? 'APS 27+ will be verified from the uploaded certificate during review.' : 'NC(V) pathway criteria passed.'} Your original subject values will be carried into your profile.</p></div><button className="text-button" type="button" onClick={() => setStep('access')}>OK — Continue to access management <span>→</span></button></div>; }

function StaffWorkspaceWithPlacements(props) {
  return <StaffWorkspace {...props} />;
}

...
function App() {
  ...
  function goApplicant(step) { setApplicantStep(step); setCheckerResult(null); }
  function checkQualification(result) { setCheckerResult(result); setSubmitted(false); setSubmittedReference(null); setApplicantStep('checker'); }
  ...
  async function submitApplication(payload = {}) {
    ...
    const applicationPayload = {
      ...
      profile,
      preferences: payload.preferences || ['Ann Latsky Campus', 'SG Lourens Campus'],
  ...
  function issueLetters() {
    void issueApiLetters('Shortlisted applicants', 'Shortlist confirmation').then((remoteState) => { if (remoteState) { setStore(remoteState); setApiConnected(true); } });
  }
  ...
  return (
    <div className="app-shell">
      ...
      {mode === 'applicant' ? (
        <MvpApplicantPortal step={applicantStep} setStep={goApplicant} pathway={checkerPathway} setPathway={setCheckerPathway} result={checkerResult} setResult={setCheckerResult} checkQualification={checkQualification} submitted={submitted} submittedReference={submittedReference} application={learnerApplication} cycle={store.cycle} submitApplication={submitApplication} recordNonQualifier={recordNonQualifier} onLogin={() => { setLoginTarget('learner'); setLoginOpen(true); }} onHelp={() => setHelpOpen(true)} session={session} onLogout={handleLogout} />
      ) : mode === 'staff' ? (
...
function MvpApplicantPortal({ step, setStep, pathway, setPathway, result, setResult, checkQualification, submitted, submittedReference, application, cycle, submitApplication, recordNonQualifier, onLogin, onHelp, session, onLogout }) {
  ...
  const [preferences, setPreferences] = useState(savedDraft.preferences || ['Ann Latsky Campus', 'SG Lourens Campus', '']);
  ...
  const canReview = intakeOpen && addressConfirmed && hasRequiredProfile && documents.id && documents.results && hasCompleteReference && preferences.some(Boolean);

  useEffect(() => {
    const draft = { pathway, pathwayValues: result?.values || {}, profile, preferences, documents, references, previousTraining, experience, addressVerified: addressConfirmed };
  ...
  }, [step, pathway, result, profile, preferences, documents, references, previousTraining, experience, addressConfirmed]);
  ...
  return <div className="applicant-layout">
    ...
    <main className="public-main">
      ...
      {step === 'profile' && <ProfileForm preferences={preferences} setPreferences={setPreferences} profile={profile} setProfile={setProfile} pathway={pathway} qualificationResult={result} addressConfirmed={addressConfirmed} setAddressConfirmed={setAddressConfirmed} documents={documents} setDocuments={setDocuments} references={references} setReferences={setReferences} previousTraining={previousTraining} setPreviousTraining={setPreviousTraining} experience={experience} setExperience={setExperience} canReview={canReview} setStep={setStep} />}
      {step === 'review' && <CompleteReviewApplicationMinimal profile={profile} pathway={pathway} preferences={preferences} documents={documents} references={references} previousTraining={previousTraining} experience={experience} submitted={submitted} submittedReference={submittedReference} submitApplication={() => submitApplication({ profile, pathway, preferences, documents, references, previousTraining, experience, addressVerified: addressConfirmed })} setStep={setStep} />}
    </main>
...
function MvpLanding({ setStep, cycle, application }) {
  return <>
    <section className="public-hero"><div className="hero-copy"><p className="eyebrow">STUDENT NURSE INTAKE 2027</p><h1>Start your nursing journey in Gauteng.</h1><p className="hero-lede">Apply for the Diploma in Nursing at Gauteng College of Nursing. Check your subjects and scores first, then complete your profile if you qualify.</p><div className="hero-actions"><button className="primary-button" onClick={() => setStep('checker')}>Check my qualification →</button><button className="quiet-button" onClick={() => setStep('checker')}>View requirements</button></div><div className="hero-note"><span className="secure-dot" /> Gauteng applicants only · No late applications</div></div><div className="hero-panel"><div className="crest-ring"><img src="/gpg-logo.png" alt="" /></div><div><strong>Diploma in Nursing</strong><span>Ann Latsky · Chris Hani Baragwanath<br />SG Lourens · Bonalesedi</span></div><div className="hero-rule" /><div className="hero-panel-row"><span>Applications close</span><strong>{cycle?.closeDate || '26 Sep 2026'}</strong></div><div className="hero-panel-row"><span>Current step</span><strong>Qualification check</strong></div></div></section>
...
function HelpPanel({ onClose }) {
  return <div className="modal-backdrop" role="presentation"><section className="help-dialog" role="dialog" aria-modal="true" aria-labelledby="help-title"><button className="modal-close" aria-label="Close help" onClick={onClose}>×</button><p className="eyebrow">HELP & FAQ</p><h2 id="help-title">GCON application help</h2><div className="help-list"><div><strong>Which pathways are supported?</strong><span>Senior Certificate, National Senior Certificate / Grade 12, and NC(V) Level 4.</span></div><div><strong>What must I upload?</strong><span>A certified copy of your ID and your statement of results or certificate in PDF, JPG, or PNG format.</span></div><div><strong>Can I track progress?</strong><span>You receive a receipt, correction requests, and a final outcome. Detailed progress tracking is not shown.</span></div><div><strong>Need support?</strong><span>Contact the GCON admissions support team through your approved intake support channel.</span></div></div><button className="outline-button" onClick={onClose}>Close help</button></section></div>;
}
...
function NonQualifierCard({ setResult }) { return <div className="nonqualifier-card"><div className="result-mark">!</div><div><strong>We could not confirm the minimum criteria</strong><p>Only your name, email and telephone number will be retained for future intake requirements. Your subjects and results are not saved.</p></div><div className="minimal-contact"><Field label="Name" value="Lerato Mokoena" /><Field label="Email" value="lerato.mokoena@email.com" /><Field label="Telephone" value="082 555 0194" /></div><button className="quiet-button" onClick={() => setResult(null)}>Review my results again</button></div>; }

function NonQualifierCardComplete({ setResult }) {
  ...
  async function saveContact() {
    setError('');
    const response = await recordApiNonQualifier(contact);
    if (!response) { setError('We could not save your contact details. Please try again.'); return; }
    setSaved(true);
  }
  return <div className="nonqualifier-card"><div className="result-mark">!</div><div><strong>We could not confirm the minimum criteria</strong><p>Only your name, email, and telephone number are retained for future intake requirements. Your subjects and results are not saved.</p></div><div className="minimal-contact"><label className="field"><span>Name</span><input value={contact.name} onChange={(event) => update('name', event.target.value)} /></label><label className="field"><span>Email</span><input type="email" value={contact.email} onChange={(event) => update('email', event.target.value)} /></label><label className="field"><span>Telephone</span><input type="tel" value={contact.telephone} onChange={(event) => update('telephone', event.target.value)} /></label></div>{error && <p className="login-error">{error}</p>}{saved ? <div className="decision-banner approved"><span>✓</span><div><strong>Contact details saved</strong><small>We retained only the minimum contact details for future intake requirements.</small></div></div> : <button className="primary-button" type="button" onClick={saveContact}>Keep my contact details</button>}<button className="quiet-button" type="button" onClick={() => setResult(null)}>Review my results again</button></div>;
}
...
function ProfileFormComplete({ cycle, preferences, setPreferences, profile, setProfile, pathway, qualificationResult, addressConfirmed, setAddressConfirmed, documents, setDocuments, references, setReferences, previousTraining, setPreviousTraining, experience, setExperience, canReview, setStep }) {
  ...
  const addExperience = () => experience.length < 3 && setExperience([...experience, { employer: '', role: '', years: '' }]);
  const updateExperience = (index, key, value) => setExperience(experience.map((item, itemIndex) => itemIndex === index ? { ...item, [key]: value } : item));
  const input = (label, key, type = 'text') => <label className="field" key={key}><span>{label}</span><input type={type} value={profile[key] || ''} onChange={(event) => updateProfile(key, event.target.value)} /></label>;
  ...
  return <div className="form-page"><StepHeader current="profile" /><div className="profile-layout"><section className="form-card"><div className="card-heading"><div><h2>Complete your profile</h2><p>Your qualification values stay in their original reported form and the fields below are saved as your draft.</p></div><span className="saved-label">Saved automatically</span></div>
    ...
    <div className="form-section"><div className="section-line"><h3>Gauteng address</h3><span className={`verified-chip ${addressConfirmed ? '' : 'unverified-chip'}`}>{addressConfirmed ? 'Map location verified' : 'Pin needs confirmation'}</span></div><div className="field-grid three">{input('Street address', 'streetAddress')}{input('Suburb / town', 'suburb')}<label className="field"><span>Province</span><select value={profile.province || ''} onChange={(event) => updateProfile('province', event.target.value)}><option>Gauteng</option><option>Other province</option></select></label></div><div className="map-box"><div className="map-grid" /><span className="map-pin">●</span><div className="map-label"><strong>{addressConfirmed ? 'Gauteng location confirmed' : 'Confirm your Gauteng location'}</strong><span>Confirm the pin before continuing.</span></div><button className="map-action" type="button" onClick={() => setAddressConfirmed((current) => !current)}>{addressConfirmed ? 'Change pin' : 'Confirm Gauteng pin'}</button></div></div>
    <div className="form-section"><div className="section-line"><h3>School and college preferences</h3><span className="required-note">Rank 1-3</span></div><div className="field-grid two">{input('School', 'school')}{input('Year of results', 'resultYear', 'number')}</div>{preferences.map((preference, index) => <div className="preference-input" key={`preference-${index}`}><span>Preference {index + 1}</span><select value={preference} onChange={(event) => updatePreference(index, event.target.value)}><option value="">No preference selected</option>{collegeCampuses.filter((campus) => !preferences.includes(campus) || campus === preference).map((campus) => <option key={campus}>{campus}</option>)}</select><small>Selection count is recorded for capacity planning.</small></div>)}</div>
     <div className="form-section"><div className="section-line"><h3>Evidence documents</h3><span className="required-note">2 required</span></div><p className="section-copy">Upload the document types shown below. Accepted formats: PDF, JPG or PNG, up to 10 MB each.</p><UploadableDocumentRow label={documentTypes[0]} required checked={documents.id} onChange={() => setDocuments((current) => ({ ...current, id: true }))} /><UploadableDocumentRow label={documentTypes[1]} required checked={documents.results} onChange={() => setDocuments((current) => ({ ...current, results: true }))} /></div>
    <div className="form-section"><div className="section-line"><h3>Additional questions</h3><span className="saved-label">Optional</span></div><div className="field-grid two"><label className="field"><span>Previous nursing training</span><select value={previousTraining} onChange={(event) => setPreviousTraining(event.target.value)}><option>No</option><option>Yes</option></select></label>{previousTraining === 'Yes' ? input('Previous training detail', 'previousTrainingDetail') : <label className="field"><span>Previous training detail</span><input value="Not required" readOnly /></label>}</div><div className="subsection-line"><strong>Work experience</strong><span>Up to 3 entries - not required</span></div>{experience.map((item, index) => <div className="experience-row" key={`experience-${index}`}><input value={item.employer} onChange={(event) => updateExperience(index, 'employer', event.target.value)} placeholder="Employer" /><input value={item.role} onChange={(event) => updateExperience(index, 'role', event.target.value)} placeholder="Role" /><input value={item.years} onChange={(event) => updateExperience(index, 'years', event.target.value)} placeholder="Years" /></div>)}<button className="quiet-button add-button" type="button" onClick={addExperience} disabled={experience.length >= 3}>+ Add work experience</button></div>
    <div className="form-section"><div className="section-line"><h3>References</h3><span className="required-note">1-3 mandatory</span></div><p className="section-copy">Add at least one reference with a relationship and telephone number.</p>{references.map((reference, index) => <div className="reference-row" key={`reference-${index}`}><input value={reference.name} onChange={(event) => updateReference(index, 'name', event.target.value)} placeholder="Reference name" /><select value={reference.relationship} onChange={(event) => updateReference(index, 'relationship', event.target.value)}>{relationshipOptions.map((relationship) => <option key={relationship}>{relationship}</option>)}</select><input value={reference.telephone} onChange={(event) => updateReference(index, 'telephone', event.target.value)} placeholder="Telephone" /></div>)}<button className="quiet-button add-button" type="button" onClick={addReference} disabled={references.length >= 3}>+ Add another reference</button></div>
  ...
  </section><aside className="profile-summary"><div className="summary-top"><span className="summary-avatar">{`${profile.firstName?.[0] || 'L'}${profile.surname?.[0] || 'M'}`}</span><div><strong>{`${profile.firstName || 'First'} ${profile.surname || 'name'}`}</strong><span>{pathway}</span></div></div><div className="summary-score"><span>Reported score</span><strong>{qualificationResult?.score || qualificationResult?.values?.aps || 'Pending'}</strong></div><div className="summary-list"><div><span>Stage</span><strong>Stage 1: Applicant</strong></div><div><span>Address</span><strong>{addressConfirmed ? 'Gauteng verified' : 'Confirm map pin'}</strong></div><div><span>Preferences</span><strong>{preferences.filter(Boolean).length} selected</strong></div><div><span>References</span><strong>{references.filter((reference) => reference.name?.trim()).length} added</strong></div></div><div className="summary-note">Your latest saved profile is used when you submit.</div></aside></div></div>;
}
...
function LettersPage({ issued, issueLetters }) { return <section className="staff-page"><div className="page-heading"><div><p className="eyebrow">LETTERS & COMMS</p><h1>Communication centre</h1><p>Issue controlled outcome and test letters. Every message is linked to the intake cycle and applicant record.</p></div><span className="review-scope"><span className="secure-dot" /> Azure-ready archive</span></div><div className="panel letter-composer"><div className="letter-controls"><label className="field"><span>Audience</span><select defaultValue="Shortlisted applicants"><option>Shortlisted applicants</option><option>Declined applicants</option><option>Withdrawn applicants</option></select></label><label className="field"><span>Letter template</span><select defaultValue="Psychometric test invitation"><option>Psychometric test invitation</option><option>Shortlist confirmation</option><option>Decline outcome</option><option>Termination / withdrawal</option></select></label><label className="field"><span>Annual cycle</span><select defaultValue="GCON 2027"><option>GCON 2027</option><option>GCON 2028</option></select></label></div><div className="letter-preview"><div className="letter-mark"><img src="/gpg-logo.png" alt="" /><strong>GCON 2027</strong></div><p>Dear Lerato Mokoena,</p><p>You have been shortlisted for the next evaluation step. Your psychometric assessment invitation will be available in your applicant communication history.</p><div className="letter-sign">Admissions coordinator<br /><strong>Gauteng College of Nursing</strong></div></div><div className="letter-actions"><span>{issued ? 'Issued to 680 applicants · recorded in audit trail' : 'Preview before issuing. The recipient list is controlled by status.'}</span><button className="primary-button" onClick={issueLetters}>{issued ? 'Issued' : 'Issue letters'} <span>→</span></button></div></div></section>; }
...
function QualificationCheckerV2({ pathway, setPathway, result, setResult, checkQualification, setStep }) {
  ...
  const select = (label, key, options) => <label className="field" key={key}><span>{label}</span><select value={values[key] || ''} onChange={(event) => update(key, event.target.value)}>{options.map((option) => <option key={option} value={option}>{option}</option>)}</select></label>;
  const number = (label, key) => <label className="field" key={key}><span>{label}</span><input type="number" min="0" max="100" value={values[key] || ''} onChange={(event) => update(key, event.target.value)} /></label>;
  const controls = pathway === 'Senior Certificate'
    ? [select('English (HG D or SG C)', 'english', ['HG D', 'HG C', 'HG B', 'HG A', 'SG C', 'SG B', 'SG A', 'Below requirement']), select('Biology (HG D or SG C)', 'biology', ['HG D', 'HG C', 'HG B', 'HG A', 'SG C', 'SG B', 'SG A', 'Below requirement']), select('Mathematics (HG D or SG C)', 'mathematics', ['HG D', 'HG C', 'HG B', 'HG A', 'SG C', 'SG B', 'SG A', 'Below requirement']), number('M score', 'mScore')]
    : pathway === 'NC(V) Level 4'
      ? [number('English First Additional Language (%)', 'englishFal'), number('Maths / Maths Literacy (%)', 'mathematics'), number('Life Orientation (%)', 'lifeOrientation'), number('SA Health Care System (%)', 'saHealthCare'), number('Public Health (%)', 'publicHealth'), number('The Human Body and Mind (%)', 'humanBody'), number('Community Oriented Primary Care (%)', 'communityPrimaryCare')]
      : [select('English level', 'english', ['4', '5', '6', '7', '3 or below']), select('Life Sciences level', 'lifeSciences', ['4', '5', '6', '7', '3 or below']), select('Mathematics level', 'mathematics', ['4', '5', '6', '7', '3 or below']), select('Maths Literacy level', 'mathsLiteracy', ['4', '5', '6', '7', 'Not taken']), number('Reported APS (LO half-weighted)', 'aps')];
  ...
  return <div className="form-page"><StepHeader current="checker" /><div className="form-grid"><section className="form-card checker-card"><div className="card-heading"><div><h2>Check your qualification</h2><p>Choose the certificate pathway and confirm only the criteria that apply to it.</p></div><span className="required-note">Required</span></div><div className="pathway-tabs">{pathways.map((item) => <button className={pathway === item ? 'selected' : ''} key={item} onClick={() => setPathway(item)}>{item}</button>)}</div><div className="field-grid two"><label className="field"><span>South African ID number</span><input value={values.idNumber || ''} onChange={(event) => update('idNumber', event.target.value)} /></label><label className="field"><span>Year of results</span><input value={values.resultYear || ''} onChange={(event) => update('resultYear', event.target.value)} /></label></div><div className="field-grid two">{controls}</div><div className="form-footnote"><span className="secure-dot" /> Original reported values are preserved. No percentage-to-APS or certificate equivalency is invented.</div>{result?.status === 'qualifies' && <QualificationResultCard result={result} setStep={setStep} />}{result?.status === 'does-not-qualify' && <NonQualifierCard setResult={setResult} />}{!result && <><button className="primary-button form-submit" onClick={evaluate}>Check qualification <span>→</span></button><button className="checker-secondary" onClick={markNotQualified}>I do not meet all of the criteria</button></>}</section><aside className="side-explain"><div className="side-icon">✓</div><h3>Why we ask this first</h3><p>Your qualification result determines whether you continue to registration. If you do not qualify, we keep only minimal contact details for future requirements.</p><div className="criteria-mini"><strong>{pathway} rule</strong><span>{pathway === 'Senior Certificate' ? 'English, Biology and Mathematics at HG D or SG C, plus M score 17.' : pathway === 'NC(V) Level 4' ? '50% in fundamentals and 60% in each named vocational subject.' : 'English and Life Sciences Level 4+, Maths Level 4 or Maths Literacy Level 5+, APS 27+.'}</span></div><div className="criteria-mini"><strong>Gauteng residence</strong><span>Your structured address and map pin must be within Gauteng.</span></div></aside></div></div>;
}

function QualificationResultCard({ result, setStep }) { return <div className="result-card"><div className="result-mark">✓</div><div><strong>You meet the {result.pathway} minimum criteria</strong><p>Recorded score: <b>{result.score}</b> · Your original subject values will be carried into your profile.</p></div><button className="text-button" onClick={() => setStep('profile')}>Continue registration <span>→</span></button></div>; }

function StaffWorkspaceWithPlacements(props) {
  if (props.page === 'placements') return <div className="staff-layout"><main className="staff-main"><div className="staff-content"><PlacementsPage applications={props.applications} assignPlacement={props.assignPlacement} withdrawApplication={props.withdrawApplication} setPage={props.setPage} /></div></main></div>;

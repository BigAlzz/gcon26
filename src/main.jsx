...
const helpContent = {
  applicant: {
    ...
    sections: [
      ...
      ['2. Complete your profile', 'Provide your contact details, South African ID, school and result year. Click the OpenStreetMap pin to fill the address, then confirm that the location is in Gauteng.'],
      ['3. Upload and review', 'Upload the two approved evidence documents as PDF, JPG, or PNG, add at least one reference, select college preferences, and review the application before submitting.'],
      ['4. After submission', 'Keep the daily reference number. You will see a receipt, correction requests, in-app notifications and the final outcome. A detailed progress tracker is not shown. Use chat to ask admissions a question.'],
    ],
  },
  staff: {
    eyebrow: 'ADMISSIONS WORKSPACE HELP',
    title: 'Run the intake process step by step',
    intro: 'Admissions owns eligibility decisions, evidence review, shortlist control, communications, placements and the audit trail. Employers only act after a staff placement offer is prepared.',
    sections: [
      ...
      ['2. Review applications', 'Open Applications or Review queue. Work from the reported pathway values and uploaded evidence. Verify each document separately, request a correction when something is unclear, or reject it with the controlled reason.'],
      ['3. Make the eligibility decision', 'Approve only after the identity, documents, Gauteng address and pathway requirements are checked. Decline with a reason when the requirements are not met. Mass decline is for eligible under-review records and creates one linked in-app notification per learner.'],
      ['4. Invite and record the psychometric test', 'Approved candidates appear in Shortlist. Select Invite psychometric test to record that the invitation was issued. The demo does not administer the test; a real test provider would send the assessment and return the result. After an invitation, record Pending, Passed, Failed or No show, with an optional score and notes. The outcome supports the next decision but does not replace eligibility or the reported academic ranking.'],
      ['5. Prepare placement offers', 'After the required review and evaluation steps, open Placements, choose a campus and select Prepare offer. The API checks capacity, records the offer and releases that candidate to the employer organisation. A full campus cannot receive another offer.'],
      ['6. Communicate and close the loop', 'In Communication centre, choose the controlled audience and template, edit the message, preview it, then issue it. Delivery batches are recorded. Learners see official outcomes and notifications in their portal. Use Audit trail to confirm decisions, notifications, offers and withdrawals.'],
    ],
  },
  employer: {
    ...
    sections: [
      ['What you can see', 'Review released or assigned candidates, their pathway, placement status, candidate contact details and the approved campus contact directory. Unreleased evidence and other organisations are not visible.'],
...
function HelpPanel({ role = 'applicant', onClose }) {
  ...
  return <div className="modal-backdrop" role="presentation"><section className="help-dialog" role="dialog" aria-modal="true" aria-labelledby="help-title"><button className="modal-close" aria-label="Close help" onClick={onClose}>Ã—</button><p className="eyebrow">{content.eyebrow}</p><h2 id="help-title">{content.title}</h2><p className="help-intro">{content.intro}</p><div className="help-list">{content.sections.map(([title, body]) => <div key={title}><strong>{title}</strong><span>{body}</span></div>)}</div><button className="outline-button" onClick={onClose}>Close help</button></section></div>;
}
...
function QualificationCheckerV2({ pathway, setPathway, result, setResult, checkQualification, setStep }) {
  ...
  const update = (key, value) => setValues((current) => {
    ...
    if (pathway === 'NSC / Grade 12' && key === 'mathematics' && value !== '3 or below') next.mathsLiteracy = 'Not taken';
    if (pathway === 'NSC / Grade 12' && key === 'mathsLiteracy' && value !== 'Not taken') next.mathematics = '3 or below';
    return next;
  });

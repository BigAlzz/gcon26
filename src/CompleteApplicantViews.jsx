...
function detailText(value, fallback = 'Not supplied') {
  return String(value || '').trim() || fallback;
}

function detailDateRange(item) {
  const start = item.startDate || '';
  const end = item.current ? 'Current' : (item.endDate || '');
  return detailText([start, end].filter(Boolean).join(' – '), 'Dates not supplied');
}
  ...
  return <div className="form-page">
    ...
    <section className="application-sheet"><div className="sheet-head"><div><p className="eyebrow">APPLICATION PREVIEW</p><h2>GCON 2027 - Diploma in Nursing</h2><p>Review your details before submitting. Your submitted application becomes a fixed snapshot.</p></div><div className="draft-tag">Draft - values preserved</div></div>
      <div className="cv-grid"><div className="cv-main">
        ...
        <section className="cv-section"><h3>Training and experience</h3><div className="cv-fields"><div><span>Previous nursing training</span><strong>{detailText(previousTraining)}</strong></div></div><div className="cv-detail-group"><h4>Training records</h4>{trainingHistory.filter((item) => item.institution?.trim() || item.qualification?.trim() || item.course?.trim()).map((item, index) => <article className="cv-detail-item" key={`training-${index}`}><strong>{detailText(item.qualification, 'Training record')}</strong><span>{detailText(item.institution)} · {detailText(item.course)}</span><small>{item.completed ? `Completed: ${item.completed}${item.year ? ` · ${item.year}` : ''}` : 'Completion not supplied'} · SANC registered: {detailText(item.sancRegistered, 'Not supplied')}</small></article>)}{!trainingHistory.some((item) => item.institution?.trim() || item.qualification?.trim() || item.course?.trim()) && <p className="section-copy">No detailed training records supplied.</p>}</div><div className="cv-detail-group"><h4>Work experience</h4>{experience.filter((item) => item.employer?.trim() || item.jobTitle?.trim() || item.role?.trim() || item.responsibilities?.trim()).map((item, index) => <article className="cv-detail-item" key={`experience-${index}`}><strong>{detailText(item.jobTitle || item.role, 'Role not supplied')}</strong><span>{detailText(item.employer)} · {detailDateRange(item)}</span>{item.responsibilities?.trim() && <small>Responsibilities: {item.responsibilities}</small>}{item.reason?.trim() && <small>Reason for leaving: {item.reason}</small>}</article>)}{!experience.some((item) => item.employer?.trim() || item.jobTitle?.trim() || item.role?.trim() || item.responsibilities?.trim()) && <p className="section-copy">No detailed work experience supplied.</p>}</div><div className="cv-detail-group"><h4>References</h4>{references.filter((reference) => reference.name?.trim()).map((reference, index) => <article className="cv-detail-item" key={`reference-${index}`}><strong>{reference.name}</strong><span>{detailText(reference.relationship)} · {detailText(reference.telephone)}</span></article>)}{!references.some((reference) => reference.name?.trim()) && <p className="section-copy">No references supplied.</p>}</div></section>
